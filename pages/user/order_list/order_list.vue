	...
	<view>
		...
		<view class="order-list">
			<view class="list">
				...
				<view class="row" v-for="(item,indexs) in list" :key="indexs">
					<!-- <view class="">
						{{item[0].orderSn}}
					</view> -->
					<view class="type">
						...
						<view class="status">
							{{typeText[item[0].status]}}
						</view>
					</view>
					<view class="row" v-for="(row,index) in item" :key="index" @tap="goOrderdetail(row.orderSn,row.status)">
					<view class="order-info">
						...
						<view class="right">
							...
							<view class="spec">{{row.productAttr | getValue}}</view>
					...
					<view class="detail">
						<view class="number">共{{item.length}}件商品</view><view class="sum">合计￥<view class="price">{{item[0].totalAmount}}</view></view>
						<!-- <view class="nominal">(含运费 ￥{{row.freight}})</view> -->
					</view>
					<view class="btns">
						<block v-if="item[0].status== 0 "><view class="default" @tap="cancelOrder(row)">取消订单</view><view class="pay" @tap="toPayment(row)">付款</view></block>
						<block v-if="item[0].status== 1"><view class="default" @tap="remindDeliver(row)">提醒发货</view></block>
						<block v-if="item[0].status== 2 "><view class="default" @tap="showLogistics(row)">查看物流</view><view class="pay">确认收货</view><view class="pay">我要退货</view></block>
						<block v-if="item[0].status== 3">
							<!-- <view class="default">评价</view> -->
							<view class="default">再次购买</view>
						</block>
						<!-- <block v-if="item[0].status==''"><view class="default">再次购买</view></block> -->
						<!-- <block v-if="item[0].status==''"><view class="default">查看进度</view></block> -->
						<block v-if="item[0].status== '4' "><view class="default">已取消</view></block>
					</view>
				</view>
			</view>
		</view>
